...
def test_make_deltas_empty_array() -> None:
    deltas = make_deltas(array([]))
    assert all(deltas == array([]))


def test_make_deltas_single_point_array() -> None:
    deltas = make_deltas(array([1.]))
    assert all(deltas == array([]))


def test_make_deltas_even_sequence() -> None:
    deltas = make_deltas(array([1., 2., 3., 4., 5., 10.]))
    assert all(deltas == array([1, 1, 1, 1, 5]))


def test_make_deltas_fluctuated_sequence() -> None:
